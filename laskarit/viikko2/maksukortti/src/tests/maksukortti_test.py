...
class TestMaksukortti(unittest.TestCase):
    ...
    def test_negatiivisen_summan_lataaminen_ei_vie_saldoa_negatiiviseksti(self):
        kortti = Maksukortti(1000)
        kortti.lataa_rahaa(-500)

        self.assertEqual(kortti.saldo_euroina(), 10.0)

    def test_kortilla_pystyy_ostamaan_edullisen_lounaan_kun_saldo_on_tasan_edullisen_hinta(self):
        kortti = Maksukortti(250)
        kortti.syo_edullisesti()

        self.assertEqual(kortti.saldo_euroina(), 0.0)

    def test_kortilla_pystyy_ostamaan_maukkaan_lounaan_kun_saldo_on_tasan_maukkaan_hinta(self):
